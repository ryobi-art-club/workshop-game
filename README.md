...
エディタのターミナルで確認します。エディタ上部のメニューから「ターミナル」→「新しいターミナル」を選ぶと、画面の下にターミナルが開きます。
...
1. フォークしたページ（`https://github.com/あなたのユーザー名/workshop-game`）で、緑色の「**Code**」ボタンを押し、表示された URL をコピーする
2. エディタのターミナルで、次を入力する（URL の部分は、コピーしたものを貼り付け）

```
git clone https://github.com/あなたのユーザー名/workshop-game.git
```

3. エディタのメニューから「ファイル」→「フォルダーを開く」を選び、できた `workshop-game` フォルダを開く

左側のファイル一覧に `sample.py` や `README.md` が表示されれば成功です。

> `git clone` をすると、ターミナルを開いた場所（多くの場合はユーザーのホームフォルダ）に `workshop-game` フォルダができます。

---

## 8. サンプルのゲームを動かす

`workshop-game` フォルダを開いた状態で、ターミナルを開き（「ターミナル」→「新しいターミナル」）、次を入力します。
...
| `can't open file ... sample.py` と出る | `workshop-game` フォルダを開けていません。手順 7 の 3 をやり直す |
...
Git の記録は「**ゲームのセーブ**」のようなものです。あとで壊れてしまっても、セーブしたところまで戻せます。
...
ターミナルで、`workshop-game` フォルダにいる状態で入力します。
...
新しく作ったファイルや変更したファイルが赤い文字で表示されます。
...
変わったファイルを全部まとめて選びたいときは、`.`（ピリオド）を使います。

```
git add .
```

もう一度 `git status` を実行すると、選んだファイルが緑色になっています。

**③ commit：記録する**

```
git commit -m "OXゲームを作った"
```

`-m` のあとの `" "` の中には、何をしたかのメモを書きます。あとで見返したときに分かるように書きましょう。
...
いままでの commit の一覧が表示されます（`q` キーで閉じます）。
...
初めての push では、GitHub へのログインを求められます。

- **ブラウザや、エディタのログイン画面が出たとき**：画面の指示に従って GitHub にログインし、アクセスを許可します
- **ターミナルで `Password:` と聞かれたとき**：GitHub のパスワードは使えません。`Ctrl + C` で中断し、エディタ左側の「**ソース管理**」アイコン（枝分かれしたマーク）を開いて「**変更の同期**」ボタンを押すと、ログイン画面が出ます
